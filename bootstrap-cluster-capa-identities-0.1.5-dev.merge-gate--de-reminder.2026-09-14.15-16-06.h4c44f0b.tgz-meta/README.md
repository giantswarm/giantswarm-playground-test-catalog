...
```sh
op run --env-file .env.tpl -- mctl create cmc-repo --customer <customer>
```
...
| Variable | Used for |
|---|---|
| `GITHUB_TOKEN` | Creating the repo, setting its secrets and committing the workflows — needs org-admin rights and the `workflow` scope |
