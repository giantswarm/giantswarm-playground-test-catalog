...
Tekton webhook, and the `mctl/ready-to-merge` required status check. Re-running
it repairs a repo that is missing any of those four — it does **not** refresh
the template's own files, since those are copied only at creation.
