...
(which carries the workflows and `op.env`) and wires up everything its CI
needs: the `giantswarm-mctlbot` App secrets, the `MCTL_IMAGE` variable, the
Tekton webhook, and the `mctl/ready-to-merge` required status check.

It is idempotent, so it also works as a repair command for a repo that already
exists but is missing some of that wiring.
...
| Variable | Used for |
|---|---|
| `GITHUB_TOKEN` | Creating the repo and setting its secrets — needs org-admin rights |
...
This is a local, human-run step: `mctl init` runs *inside* the CMC repo in CI,
so the repo has to exist first, and the CI token has no org-admin rights to
create it. The configs (CCR) repository is not created here — `mctl config`
creates it on the first run for a customer.
